...
export function registerRoutes(app: FastifyInstance): void {
  ...
  app.post(
    ...
    async (request: FastifyRequest) => {
      const req = request as AuthenticatedRequest;
      const body = schemas.CreateChannelRequestSchema.parse(request.body);
  ...
  app.get(
    ...
    async (request: FastifyRequest) => {
      const req = request as AuthenticatedRequest;
      const channels = await channelsService.getChannelsForMember(
        req.user.sub,
        req.user.type
      );
      return { channels };
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply) => {
      ...
      const messages = await channelsService.getChannelMessages(
        id,
